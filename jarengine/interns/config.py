...
class JEInternConfig(_JTKInternConfig):
    ...
    base_path = join(getcwd(), ".je-config/")

    def __init__(self, name):
        """JEInternConfig creator"""
        if not exists(JEInternConfig.base_path):
            mkdir(JEInternConfig.base_path)
        super().__init__(JEInternConfig.base_path, {"INFO": {"name": name}}, file_name=f"je-{name}.ini")
